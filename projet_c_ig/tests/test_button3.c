...
ei_bool_t button_press(ei_widget_t* widget, ei_event_t* event, void* user_param)
{
	...
	ei_anchor_t ancre;
	srand(time(NULL));	
	ancre=rand()%(ei_anc_northwest+1);
	int button_height,button_width;
	button_height=10+rand()%500;
	button_width=20+rand()%500;
	ei_size_t s={button_width,button_height};
	printf("boutton real size {%i,%i}\n",s.width,s.height);
		ei_button_configure	(widget, &s, NULL,
				 NULL, NULL, NULL, NULL, NULL, NULL, &ancre,
				 NULL, NULL, NULL, NULL, NULL);
	int		button_x		= 150;
	int		button_y		= 200;

	ei_place(widget, NULL, &button_x, &button_y, NULL, NULL, NULL, NULL, NULL, NULL );
	return EI_TRUE;
}
