                ...
                ei_rect_t * clipper)
{
        ...
        ei_draw_rectangle(surface, &bord_color, &bord, clipper);

        /* Dessin du content_rect */
        ei_draw_rectangle(surface, &toplevel->color, toplevel->widget.content_rect, clipper);
        ...
        //Amélioration
        ei_draw_rectangle(surface, &toplevel->bar_color, &bar, clipper);
        ...
        if (toplevel->resizable) {
                ...
                btn_r.top_left = plus(rec.top_left,
                                      width - toplevel->resize_size - 1,
                                      height - toplevel->resize_size - 1);

                ei_button_draw_loc(surface, btn_r, btn_r_color,
                                   ei_relief_none, 0, 0, clipper);
        }
        if (toplevel->title) {
                /* Affichage du titre */
                ei_rect_t rec_txt;
                ...
                rec_txt.size.width = toplevel->widget.screen_location.size.width - marge_txt;
                ...
                if (inter) 
                        ei_insert_text(surface, rec_txt, toplevel->title, toplevel->title_font,
                                       toplevel->title_color, 1, inter);
                 ...
                 ei_rect_t * img_rect, ei_anchor_t img_anchor, ei_rect_t * clipper)
{
        ...
        if (clipper) {
                ...
                /* Reduce the destination clipper so that */
                /* it does not overlap the raw clipper */
                ei_rect_t *inter = ei_rect_intersection(&rec_dst, clipper);
                if (inter) {

                        /* We add the raw clipper's intersecting difference to the image's clipper */
                        // img_part = img_part + (inter - rec_dst)
                        img_part.top_left.x += inter->top_left.x - rec_dst.top_left.x;
                        img_part.top_left.y += inter->top_left.y - rec_dst.top_left.y;
                        img_part.size.width += inter->size.width - rec_dst.size.width;
                        img_part.size.height += inter->size.height - rec_dst.size.height;
        ...
        /* Make sure the image clipper's size is the same as its destination */
        img_part.size = rec_dst.size;


        /* If valid clippers, draw */
